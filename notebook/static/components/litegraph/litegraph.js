...
(function(global) {
    ...
    LGraph.prototype.topologicalSort = function(nodes, links) {
        var adjacencyList = {};
        var indegree = {};
        for(var i=0; i < nodes.length; ++i){
            adjacencyList[nodes[i].id] = [];
        ...
        for(var i=0; i < nodes.length; ++i){
            if (indegree[nodes[i].id] === 0)
                zeroIndegreeNodes.push(nodes[i].id);
        }

        var ordered_id = [];
        var count = 0;
        while(zeroIndegreeNodes.length !== 0){
            var node_id = zeroIndegreeNodes.shift();
            ordered_id.push(node_id);
            count++;
            for(var i=0; i < adjacencyList[node_id].length; ++i){
                var output_node_id = adjacencyList[node_id][i];
                indegree[output_node_id]--;
                if(indegree[output_node_id] === 0)
                    zeroIndegreeNodes.push(output_node_id)
            }
        }
     ...
     * @return 'string'
     */
    LGraph.prototype.sourceCode = function() {
        var import_libraries = new Set();
        var source_code = [];
        var ordered_ids = this.topologicalSort(this._nodes, this.links)
        for (var i = 0, l = ordered_ids.length; i < l; ++i) {
            var node = this.getNodeById(ordered_ids[i]);
            console.log(node.title);
            if (node.fromlibrary !== null || node.fromlibrary !== undefined){
                import_libraries.add(node.fromlibrary)
            }
            source_code.push(node.sourceCode());
        }

        var libraries = Array.from(import_libraries, l => l);
...
(function(global) {
    ...
    function ImageIOImRead() {
		this.addOutput("image", "net.imagej.ImageJ");
        this.addProperty("value", "net.imagej.ImageJ");
        this.widget = this.addWidget("text", "image uri", '', "value");
        this.widgets_up = true;
        this.size = [140, 50];
        this.fromlibrary ='import imagej\n ij = imagej.init()\n' + 'ij.dispose()';
    }
    ...
    ImageIOImRead.prototype.sourceCode = function() {
        return `${this.getOutputSlotName(0)} = ij.io().open(r'${this.properties["value"]}')`;
    };

	ImageIOImRead.prototype.setValue = ImageIOImRead.prototype.setValue;

    LiteGraph.registerNodeType("PyimageJ/imread", ImageIOImRead);

    function ImageIOImWrite() {
        this.addInput("image", "net.imagej.ImageJ")
        this.addProperty("value", "");
        this.widget = this.addWidget("text", "uri", '', "value");
        this.widgets_up = true;
        this.size = [140, 50];
    ...
    ImageIOImWrite.prototype.sourceCode = function() {
        var image = this.getInputNodeSlotName(0);
        return `ij.io().save(r'${this.properties["value"]}', ${image})`;
    };
    ...
    LiteGraph.registerNodeType("PyimageJ/Gaussian2DFilter", ImageGaussianFilter);

    function Scale() {
        this.addInput("image", "net.imagej.ImageJ");
        this.addInput("distance", "number");
        this.addInput("known", "number");
        this.addInput("unit", "string");
        this.addOutput("image", "net.imagej.ImageJ");
        this.addProperty("distance", 36);
        this.addProperty("known", 1);
        this.addProperty("unit", "µm");
        this.size = [180, 80];
        this.fromlibrary ='from scipy.ndimage import gaussian_filter';
    }
    Scale.title = "Scale";
    // ImageGaussianFilter.desc = "Gaussian filter";
    Scale.prototype.sourceCode = function() {
        var image = this.getInputNodeSlotName(0);
        return `${this.getOutputSlotName(0)} = gaussian_filter(${image}, sigma=1)`;
    };

    LiteGraph.registerNodeType("PyimageJ/Scale", Scale);
    ...
    MakeRectangle.title = "makeRectangle";
    // ImageGaussianFilter.desc = "Gaussian filter";
    MakeRectangle.prototype.sourceCode = function() {
        var image = this.getInputNodeSlotName(0);
        return `${this.getOutputSlotName(0)} = gaussian_filter(${image}, sigma=1)`;
    };

    LiteGraph.registerNodeType("PyimageJ/makeRectangle", MakeRectangle);
    ...
    Crop.title = "Crop";
    // ImageGaussianFilter.desc = "Gaussian filter";
    Crop.prototype.sourceCode = function() {
        var image = this.getInputNodeSlotName(0);
        return `${this.getOutputSlotName(0)} = gaussian_filter(${image}, sigma=1)`;
    };

    LiteGraph.registerNodeType("PyimageJ/Crop", Crop);
    ...
    to_8_bit.title = "to 8-bit";
    // ImageGaussianFilter.desc = "Gaussian filter";
    to_8_bit.prototype.sourceCode = function() {
        var image = this.getInputNodeSlotName(0);
        return `${this.getOutputSlotName(0)} = gaussian_filter(${image}, sigma=1)`;
    };

    LiteGraph.registerNodeType("PyimageJ/to_8_bit", to_8_bit);
    ...
    Mean.title = "Mean";
    // ImageGaussianFilter.desc = "Gaussian filter";
    Mean.prototype.sourceCode = function() {
        var image = this.getInputNodeSlotName(0);
        return `${this.getOutputSlotName(0)} = gaussian_filter(${image}, sigma=1)`;
    };

    LiteGraph.registerNodeType("PyimageJ/Mean", Mean);
...
(function(global) {
	...
	MathOperation.prototype.sourceCode = function() {
        var A = this.getInputNodeSlotName(0);
        if (A === undefined || A === null) {
            A = this.properties["A"];
        }
        var B = this.getInputNodeSlotName(1);
        if (B === undefined || B === null) {
            A = this.properties["B"];
        }

        if(this.getOutputNodes(0))
            return `${this.getOutputSlotName(0)} = ${A} + ${B}`;
        return `${A} + ${B}`;
    };
