...
function createDialog(dialog_id, title='Dialog Title...', content = null){
...
<!-- width and height are optional.-->
	if(document.getElementById(dialog_id))
		throw "The dialog id is not unique";
	...
	var content_containter = document.createElement('div');
	content_containter.className = 'content';
	content_containter.style.zIndex = dialog.style.zIndex + 1;
	if(content != null)
		content_containter.appendChild(content);
	...
	buttonset.appendChild(okbutton);
	buttonset.appendChild(cancelbutton);
	buttonpane.appendChild(buttonset);

	dialog.appendChild(titlebar);
	dialog.appendChild(close_button);
	dialog.appendChild(content_containter);
	dialog.appendChild(buttonpane);
	document.body.appendChild(dialog)
}

function showDialog(show_dialog_button, dialog_id, title, content, status) {
	if(!show_dialog_button)
		throw 'Please pass clicked button as a parameter';
	show_dialog_button.disabled = true;
	if(status != undefined || status != null)
		status.textContent = 'Dialog showed...';
	var dialog = dialogs[dialog_id]
	if (!dialog) {
		createDialog(dialog_id, title, content)
	...
	function callbackDialog(btnName) {
		show_dialog_button.disabled = false;
		show_dialog_button.focus();
		if(status != undefined || status != null)
			if (btnName == 'close')
				status.textContent = 'Dialog hidden...';
			else
				status.textContent = btnName + ' button clicked...';
	}
}
...
var	_minW = 100, // The exact value get's calculated
	...
	// const contentResizeEvent = new CustomEvent('ContentResize', {
	// 	bubbles: false,
	// });
	...
	*/
	...
	_setDialogContent = function() {
		...
		_dialogContent.querySelector('canvas').width = w;
		_dialogContent.querySelector('canvas').height = h;
